...
export class EventBus {
    ...
    public constructor(intMng: interaction.InteractionManager) {
        intMng.addListener("pointerdown", (data: any) => {
            this.events.pointerdown.emit(data.target, data);
        });
        intMng.addListener("pointerup", (data: any) => {
            this.events.pointerup.emit(data.target, data);
        });
        intMng.addListener("pointermove", (data: any) => {
            this.events.pointermove.emit(data.target, data);
        });
        intMng.addListener("pointerover", (data: any) => {
            this.events.pointerover.emit(data.target, data);
        });
        intMng.addListener("pointerout", (data: any) => {
            this.events.pointerout.emit(data.target, data);
        });
    }
}
