...
export class TimelineView extends Drawable<ITimelineViewProps> {
    ...
    public setup() {
        ...
        this.viewInstance.eventBus.events.keydown.subscribe(this, (ev) => {
            ...
            if (ev.key === "Delete") {
                this.getSelectedItems().forEach((i) => this.props.editor.removeItem(i));
            }
        });
        ...
        this.viewInstance.eventBus.events.resize.subscribe(this, () => {
        ...
        this.viewInstance.eventBus.events.removeTrack.subscribe(this, (track) => {
            this.props.editor.items
                .filter((i) => i.trackId === track.id)
                .forEach((i) => this.props.editor.removeItem(i));
        ...
        this.tracks.onNewItem = (newTrack) =>
            this.createView(TrackView, { track: newTrack, headerWidth: this.props.trackHeaderWidth });
        this.tracks.onRender = (trackView, t, i) => {
    ...
    private onMousedown(ev: interaction.InteractionEvent) {
        ...
        if (!this.ctrlPressed) {
            this.props.editor.items.forEach((i) => {
                i.selected = false;
            });
        }
        ...
        if (this.isDragging) {
            if (this.dragItem) {
                if (this.dragArea === "leftHandle" || this.dragArea === "rightHandle") {
                    ...
                    const diffUnits = Math.floor(
                        (x - this.dragStartPosition.x) / this.viewInstance.positionCalculator.unitWidth
                    );
                    let diffTracks = 0;
    ...
    private onItemMousedown(item: Item, area: ItemArea, data: interaction.InteractionEvent) {
        ...
        this.isDragging = true;
        this.dragStartStates = this.getSelectedItems().map((i) => ({
